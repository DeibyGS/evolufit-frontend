...
export const RMCalculator = () => {
  ...
  const calculateRM = (e) => {
    e.preventDefault();
    if (!weight || !reps || reps <= 0) return toast.error("Introduce peso y repeticiones válidas");
    const w = parseFloat(weight);
    const r = parseInt(reps);
    setResults({ epley: (w * (1 + r / 30)).toFixed(1), brzycki: (w / (1.0278 - 0.0278 * r)).toFixed(1) });
  ...
  const handleSaveRM = async () => {
    ...
    const previousBest = savedRMs.filter(rm => rm.exerciseName === selectedExercise).reduce((prev, current) => (prev.brzyckiResult > current.brzyckiResult) ? prev : current, null);

    if (!previousBest || currentBrzycki > previousBest.brzyckiResult) {
      if (previousBest) {
        await Swal.fire({
          title: '¡NUEVO RÉCORD PERSONAL! 🔥',
          text: `Has superado tu marca anterior.`,
          icon: 'success',
    ...
    const payload = {
      exerciseName: selectedExercise, muscleGroup: selectedGroup,
      weightUsed: Number(weight), repsDone: Number(reps),
      epleyResult: Number(results.epley), brzyckiResult: Number(results.brzycki)
    };
    ...
    try {
      ...
      if (response.ok) {
        ...
        setIsCalculated(false); setResults(null); fetchSavedRMs(); 
      }
